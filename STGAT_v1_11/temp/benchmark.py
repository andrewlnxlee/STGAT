...
def run_benchmark():
    ...
    if not os.path.exists(config.MODEL_USE_PATH): print("Model weights not found, skipping GNN eval."); gnn_model = None
    else: gnn_model.load_state_dict(torch.load(config.MODEL_USE_PATH, map_location=device)); gnn_model.eval()
